...
import static com.sinux.stream.loader.StreamLoadConstants.RESULT_STATUS_FAILED;
...
public class DefaultStreamLoader implements StreamLoader, Serializable {
    ...
    protected StreamLoadResponse sendToSR(TableRegion region) {
        try {
            ...
            httpPut.addHeader("format",region.getProperties().getDataFormat().name());
          ...
          //  log.info("put body:{}", IoUtil.read(httpPut.getEntity().getContent()));
